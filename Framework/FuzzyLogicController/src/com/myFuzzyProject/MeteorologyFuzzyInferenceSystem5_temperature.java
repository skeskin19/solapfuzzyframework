...
public class MeteorologyFuzzyInferenceSystem5_temperature {
	...
	private static final String PREDICTION_FILE_NAME = "meteorological-rf-predictions-2.csv";
	...
	public static void main(String[] args) throws Exception {
		...
		String filename = "meteorology-26052020.fcl";//"meteorology-17072019.fcl";//meteorology_7_s005_c05
		...
		System.out.println("Day-Month-Year\tlatitude\tlongitude\taltitude\tActual-Temperature\tPredicted-Temperature\tRamdomForest-Prediction\tDiff\tDiff-ratio");
		...
		for(MetDataBean mb:records) {
			...
			double predictedDefuzzifiedData = functionBlock.getVariable("temperature_out").getValue();
	...
	private static List<MetDataBean> loadMeteorologicalData() {
		...
		try (BufferedReader br = new BufferedReader(new FileReader(FILE_NAME))) {
		    ...
		    while ((line = br.readLine()) != null) {
		        ...
		        records.add(new MetDataBean(values[0], values[1], values[2], values[3], values[4], values[5], values[6], values[7], values[8], values[9], values[10], values[11], values[12], values[13], values[14], values[15]));
		    }
		} catch (Exception e) {
			e.printStackTrace();
