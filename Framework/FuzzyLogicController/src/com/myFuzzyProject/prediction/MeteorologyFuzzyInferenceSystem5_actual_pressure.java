...
public class MeteorologyFuzzyInferenceSystem5_actual_pressure {
	private static final String FILE_NAME = "meteorological-data-sta-2016.csv";
	private static final String PREDICTION_FILE_NAME = "meteorological-rf-actual-pressure-predictions.csv";
	...
	public static void main(String[] args) throws Exception {
		...
		for(MetDataBean mb:records) {
			...
			double predictedDefuzzifiedData = functionBlock.getVariable("actual_pressure_out").getValue();
	...
	private static List<MetDataBean> loadMeteorologicalData() {
		...
		try (BufferedReader br = new BufferedReader(new FileReader(FILE_NAME))) {
		    ...
		    while ((line = br.readLine()) != null) {
		        ...
		        records.add(new MetDataBean(values[0], values[1], values[2], values[3], values[4], values[5], values[6], values[7], values[8], values[9], values[10], values[11], values[12], values[13], values[14], values[7]));
		    }
		} catch (Exception e) {
			e.printStackTrace();
