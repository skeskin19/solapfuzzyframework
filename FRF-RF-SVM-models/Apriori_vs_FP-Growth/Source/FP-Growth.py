...
import os
import psutil
print('memory status at startup: ', psutil.virtual_memory())

process = psutil.Process(os.getpid())
memory_start=process.memory_info().rss # in bytes 
print('process_id: ', os.getpid(), ' uses memory at start: ', memory_start) 

#Read data from CSV
data = pd.read_csv('../met-data-train-for-association-fuzzy.csv')
...
def parsePredication(predication, typ):
    rlStr="";
    if(isinstance(predication, tuple)):
        for ind in range(0,len(predication)):
            prt = predication[ind].split("<")
            cls=prt[0]
            memShp=prt[1].split(">")
            if(len(rlStr)==0):
                rlStr = cls + typ + " IS " + memShp[0] + typ
            else:
                rlStr = rlStr + " AND "+ cls + typ + " IS " + memShp[0] + typ
    else:
        prt = predication.split("<")
        cls=prt[0]
        memShp=prt[1].split(">")
        rlStr = cls + typ + " IS " + memShp[0] + typ
    return rlStr
...
print('memory status at association rules creation: ', psutil.virtual_memory())
ant=count.copy()
blockCnt=0
rlCnt=0
                    ...
                    if((ant[len(j)][j]/ant[l][n])>=minconf):
                        ...
                        if(closed.__contains__((n))):
                            ...
                            if(len(r)==1):
                                ...
                                #rlCnt=rlCnt+1
                                #print("\t RULE ",rlCnt," : ","IF ",parsePredication(n, '_in')," THEN ",parsePredication(r2, '_out'),";")
                            else:
                                print(n,"(",ant[l][n],")","--->",r,"(",ant[len(r)][r],")", " conf(",(ant[len(j)][j]/ant[l][n]),")")
                                #print("----------sk-rule-print-start---------------")
                                #for ind in range(0,len(r)):
                                #    rlCnt=rlCnt+1
                                #    print("\t RULE ",rlCnt, ": ","IF ",parsePredication(n, '_in')," THEN ",parsePredication(r[ind], '_out'),";")
                                #print("----------sk-rule-print-end---------------")
...
print('number of generated rules:', rlCnt)
print('memory status at end of execution: ', psutil.virtual_memory())
currentDT2 = datetime.datetime.now()
print ('end time:', str(currentDT2))
memory_end=process.memory_info().rss # in bytes 
print('process_id: ', os.getpid(), ' uses memory at end: ', memory_end)
print('memeory usage:', (memory_end-memory_start))
print('duration:', (currentDT2-currentDT))
